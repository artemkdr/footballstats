    ...
    const createUser = async () => {
		...
		if (response.ok) {
            if (responseJson?.username != null) {
                toast({ title: t('Message.CreateUserSuccess'), status: 'success' });
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} closeOnOverlayClick={false}>
            ...
            <ModalContent>
                ...
                <ModalBody>                    
                    <VStack spacing={5}>
                        ...
                        <InputInlineLabel 
                            ...
                            input={
                                <Select name={"Status"} placeholder={t("Players.Status")} value={user.Status} onChange={handleChange}>
                                    {Object.values(UserStatus).map((type) => (
                                        <option key={type} value={type}>{t('UserStatus.' + type)}</option>
                                    ))}
                                </Select>
                            }
                            />
                ...
                <ModalFooter>
                    <Button colorScheme="green" mr={3} onClick={() => { createUser() }} isDisabled={!isValid}>{t('Create')}</Button>
